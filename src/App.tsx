...
const RepRewardToast: React.FC = () => {
  ...
  return (
    <div className="dc-rep-reward" key={reward.id} role="status" aria-live="polite">
      <span className="dc-rep-reward-particle" aria-hidden="true" /><span className="dc-rep-reward-particle" aria-hidden="true" /><span className="dc-rep-reward-particle" aria-hidden="true" /><span className="dc-rep-reward-particle" aria-hidden="true" /><span className="dc-rep-reward-particle" aria-hidden="true" /><span className="dc-rep-reward-particle" aria-hidden="true" />
      <div className="dc-rep-reward-card"><span className="dc-rep-reward-kicker">REPUTATION UPDATED</span><div className="dc-rep-reward-amount"><span>+{reward.amount}</span> REP</div><span className="dc-rep-reward-caption">CONTRIBUTION LOGGED</span></div>
    </div>
  );
};
...
const MainContent: React.FC = () => {
  ...
  if (loadingAuth && needsAuthHydration) {
    return <div className="dc-app-shell min-h-screen bg-background text-on-background flex flex-col items-center justify-center p-6 space-y-4"><div className="w-12 h-12 border-4 border-primary border-t-transparent rounded-full animate-spin" /><p className="font-label-mono text-sm text-on-surface-variant">Verifying DevCollective session...</p></div>;
  }

  if (isProtected && !user) {
    return <div className="dc-app-shell min-h-screen bg-background text-on-background"><Navbar /><div className="max-w-md mx-auto mt-12 p-6 bg-surface-container border-2 border-outline-variant rounded-xl text-center space-y-4"><h2 className="font-headline-md text-2xl font-bold">Authentication Required</h2><p className="text-sm text-on-surface-variant">Please log in with your email and password to access this page.</p><button onClick={() => setActiveTab('login')} className="w-full py-3 bg-primary text-on-primary font-bold border-2 border-outline-variant dc-hard-shadow-sm">Go to Login</button></div></div>;
  }
