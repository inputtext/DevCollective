...
export const LoginPage: React.FC = () => {
  const { setActiveTab, loginWithEmail, triggerOAuthLogin, oauthInfo, authRedirectError, clearAuthRedirectError, requestPasswordReset, resetPassword } = useAuth();
      ...
      <section className="flex-1 flex items-center justify-center px-4 sm:px-8 py-20 md:py-12">
        <div className="w-full max-w-lg">
          <div className="bg-surface-container rounded-2xl border-2 border-outline-variant p-8 sm:p-10 shadow-2xl">
            ...
            {/* OAuth redirect error banner (shown if a Google/GitHub login attempt failed) */}
            {authRedirectError && (
              <div className="mb-6 p-4 bg-error/10 border-2 border-error/50 rounded-xl flex items-center gap-3 text-error text-xs font-label-mono">
                ...
                <button onClick={clearAuthRedirectError} className="text-error/70 hover:text-error">✕</button>
              </div>
            )}
                  ...
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center">
                      ...
                      <button
                        ...
                        className="font-label-mono text-[11px] uppercase text-primary hover:underline"
                      >
                        Forgot Password?
                      </button>
                    </div>
            ...
            {view === 'forgot-email' && (
              <>
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs font-label-mono uppercase text-on-surface-variant hover:text-white mb-6 transition-colors"
                >
                ...
                <header className="mb-8 text-center space-y-2">
                  <div className="w-12 h-12 mx-auto rounded-xl bg-primary/20 border-2 border-primary/40 flex items-center justify-center text-primary mb-3">
                    <Mail className="w-6 h-6" />
                  </div>
                  <h2 className="font-headline-lg text-2xl font-bold text-white">Reset your password</h2>
                  <p className="font-body-md text-sm text-on-surface-variant">
            ...
            {view === 'forgot-code' && (
              <>
                ...
                <header className="mb-8 text-center space-y-2">
                  ...
                  <h2 className="font-headline-lg text-2xl font-bold text-white">Enter your code</h2>
                  <p className="font-body-md text-sm text-on-surface-variant">
                    Check <span className="text-white font-bold">{resetEmail}</span> for a 6-digit code, then set a new password.
                  </p>
                </header>
                ...
                <form onSubmit={handleResetPassword} className="space-y-5">
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      placeholder="123456"
                      className="w-full bg-surface-container-lowest border-2 border-outline-variant rounded-xl px-5 py-3.5 font-body-md text-white tracking-[0.3em] text-center text-lg placeholder:text-outline focus:outline-none focus:border-secondary transition-all"
                      required
                    />
                  </div>
