...
export const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (signInError) {
        ...
        // Per the requested product flow, credential mismatch sends the user to registration.
      ...
      if (signIn.status === 'needs_client_trust') {
        ...
        setError('Clerk requires an additional device verification method for this account. Disable Device Trust in Clerk to use direct password login.');
        return;
      }

      if (signIn.status === 'needs_second_factor') {
        setError('This account has MFA enabled. Direct password login requires MFA to be disabled for this account.');
        return;
      }
  ...
  const renderError = () => {
    const message = error || clerkErrorMessage;
          ...
          {verificationMode ? (
            <form onSubmit={handleVerify} className="space-y-7">
              ...
              <input
                ...
                placeholder="000000"
                className="w-full h-[68px] bg-surface border-2 border-outline-variant px-5 text-2xl tracking-[0.35em] text-on-surface"
                required
              />
              {renderError()}
              <button type="submit" disabled={isBusy} className="w-full h-[68px] dc-hard-shadow-sm bg-primary text-on-primary border-2 border-outline-variant font-black uppercase text-sm flex items-center justify-center gap-4 disabled:opacity-50">
                {isBusy ? 'VERIFYING...' : 'VERIFY & CONTINUE'} <CheckCircle2 className="w-5 h-5" />
              </button>
              <button type="button" onClick={resetLogin} className="w-full dc-mono text-[10px] uppercase tracking-[0.14em] text-on-surface-variant hover:text-on-surface">
                Start over
              </button>
            </form>
          ) : forgotMode ? (
            <div className="space-y-7">
              ...
              {forgotStep === 'email' && (
                <form onSubmit={startForgotPassword} className="space-y-5">
                  ...
                  <input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="architect@devcollective.edu" autoComplete="email" className="w-full h-[68px] bg-surface border-2 border-outline-variant px-5 text-lg text-on-surface" required />
                  {renderError()}
                  <button type="submit" disabled={isBusy} className="w-full h-[68px] dc-hard-shadow-sm bg-primary text-on-primary border-2 border-outline-variant font-black uppercase text-sm flex items-center justify-center gap-4 disabled:opacity-50">
                    {isBusy ? 'SENDING...' : 'SEND RESET CODE'} <ArrowRight className="w-5 h-5" />
                  </button>
                </form>
              )}
              ...
              {forgotStep === 'code' && (
                <form onSubmit={verifyForgotCode} className="space-y-5">
                  ...
                  <input value={forgotCode} onChange={(event) => setForgotCode(event.target.value)} inputMode="numeric" autoComplete="one-time-code" placeholder="000000" className="w-full h-[68px] bg-surface border-2 border-outline-variant px-5 text-2xl tracking-[0.35em] text-on-surface" required />
                  {renderError()}
                  <button type="submit" disabled={isBusy} className="w-full h-[68px] dc-hard-shadow-sm bg-primary text-on-primary border-2 border-outline-variant font-black uppercase text-sm flex items-center justify-center gap-4 disabled:opacity-50">
                    {isBusy ? 'VERIFYING...' : 'VERIFY CODE'} <ArrowRight className="w-5 h-5" />
                  </button>
                </form>
              )}
              ...
              {forgotStep === 'password' && (
                <form onSubmit={submitNewPassword} className="space-y-5">
                  ...
                  <div className="relative">
                    <input type={showPassword ? 'text' : 'password'} value={newPassword} onChange={(event) => setNewPassword(event.target.value)} autoComplete="new-password" placeholder="New password" className="w-full h-[68px] bg-surface border-2 border-outline-variant px-5 pr-16 text-lg text-on-surface" required />
                    <button type="button" onClick={() => setShowPassword((current) => !current)} className="absolute right-5 top-1/2 -translate-y-1/2 text-on-surface-variant" aria-label={showPassword ? 'Hide password' : 'Show password'}>
                      {showPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                    </button>
                  </div>
                  {renderError()}
                  <button type="submit" disabled={isBusy} className="w-full h-[68px] dc-hard-shadow-sm bg-primary text-on-primary border-2 border-outline-variant font-black uppercase text-sm flex items-center justify-center gap-4 disabled:opacity-50">
                    {isBusy ? 'UPDATING...' : 'UPDATE PASSWORD'} <CheckCircle2 className="w-5 h-5" />
                  </button>
                </form>
              )}
            ...
            <form onSubmit={handleSubmit} className="space-y-0">
              <div className="space-y-2">
                ...
                <input
                  ...
                  autoComplete="email"
                  className="w-full h-[68px] bg-surface border-2 border-outline-variant px-5 text-lg text-on-surface"
                  required
                />
              </div>
              ...
              <div className="mt-8 space-y-2">
                <div className="flex justify-between items-center gap-4">
                  ...
                  <button type="button" onClick={() => { setForgotMode(true); setForgotStep('email'); setError(null); }} className="dc-mono text-[10px] uppercase tracking-[0.12em] text-primary hover:underline">
                    Forgot Password?
                  </button>
                </div>
                <div className="relative">
                  <input
                    ...
                    className="w-full h-[68px] bg-surface border-2 border-outline-variant px-5 pr-16 text-lg text-on-surface"
                    required
                  />
                  <button type="button" onClick={() => setShowPassword((current) => !current)} className="absolute right-5 top-1/2 -translate-y-1/2 text-on-surface-variant hover:text-on-surface" aria-label={showPassword ? 'Hide password' : 'Show password'}>
                    {showPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                  </button>
                </div>
              </div>
              ...
              <button type="submit" disabled={isBusy} className="mt-8 w-full h-[68px] dc-hard-shadow-sm bg-primary text-on-primary border-2 border-outline-variant font-black uppercase text-sm flex items-center justify-center gap-4 disabled:opacity-50">
        ...
        {!verificationMode && !forgotMode && (
          <>
            ...
            <div className="grid grid-cols-2 gap-5">
              <button type="button" onClick={() => triggerOAuthLogin('google')} className="h-[68px] flex items-center justify-center gap-3 bg-surface border-2 border-outline-variant dc-mono text-[10px] uppercase font-bold hover:bg-dc-yellow transition-colors">
                <span className="text-xl font-black" aria-hidden="true">G</span> GOOGLE
              </button>
              <button type="button" onClick={() => triggerOAuthLogin('github')} className="h-[68px] flex items-center justify-center gap-3 bg-surface border-2 border-outline-variant dc-mono text-[10px] uppercase font-bold hover:bg-dc-mint transition-colors">
                <Github className="w-5 h-5" /> GITHUB
              </button>
            </div>

            <div className="mt-10 pt-6 border-t-2 border-outline-variant text-center">
              <p className="text-sm text-on-surface-variant">
                Don't have an account?{' '}
                <button type="button" onClick={goToRegister} className="text-primary font-bold uppercase text-xs ml-1 hover:underline">Register Here</button>
              </p>
            </div>
          </>
        )}
