...
export const LeaderboardPage: React.FC = () => {
  ...
  return <div className="space-y-8 pb-16">
    <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b-2 border-outline-variant pb-7">
      ...
      <div className="flex bg-surface border-2 border-outline-variant w-fit">{(['weekly','monthly','alltime'] as const).map((timeframe) => <button key={timeframe} onClick={() => setSelectedTimeframe(timeframe)} className={`px-4 py-2 font-label-mono text-[10px] uppercase font-bold ${selectedTimeframe === timeframe ? 'bg-primary text-on-primary' : ''}`}>{timeframe}</button>)}</div>
    </header>

    {user && <section className="border-2 border-outline-variant bg-surface p-6 shadow-[5px_5px_0_#171717]"><div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5"><div className="flex items-center gap-4">{user.avatar ? <img src={user.avatar} alt={user.name} className="w-16 h-16 border-2 border-outline-variant object-cover" /> : <div className="w-16 h-16 border-2 border-outline-variant bg-dc-blue flex items-center justify-center font-bold">{user.name.slice(0,1)}</div>}<div><h3 className="font-bold text-lg">{user.name}</h3><p className="font-label-mono text-[10px] uppercase text-on-surface-variant">LEVEL {user.level} · {user.branch || 'BRANCH NOT SET'}</p><p className="font-label-mono text-[10px] text-on-surface-variant">{user.rep.toLocaleString()} REP · {user.streakDays}d <Flame className="inline w-3 h-3" /></p></div></div><button className="border-2 border-outline-variant bg-surface px-5 py-3 font-label-mono text-[10px] uppercase font-bold flex items-center gap-2"><Share2 className="w-4 h-4" /> Share Rank</button></div></section>}

    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"><div className="flex flex-wrap gap-2">{branches.map((branch) => <button key={branch} onClick={() => setSelectedBranch(branch)} className={`px-3 py-2 border-2 border-outline-variant font-label-mono text-[10px] uppercase font-bold ${selectedBranch === branch ? 'bg-primary text-on-primary' : 'bg-surface'}`}>{branch}</button>)}</div><div className="relative w-full sm:w-72"><Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant" /><input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search real members..." className="w-full bg-surface border-2 border-outline-variant pl-9 pr-4 py-3 text-xs font-label-mono" /></div></div>
