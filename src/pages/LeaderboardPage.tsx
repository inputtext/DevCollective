...
export const LeaderboardPage: React.FC = () => {
  ...
  return <div className="space-y-8 pb-16">
    ...
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"><div className="flex flex-wrap gap-2">{branches.map((branch) => <DcButton key={branch} type="button" onClick={() => setSelectedBranch(branch)} variant={selectedBranch === branch ? 'primary' : 'secondary'} className="px-3 py-2">feature/developer-note-fluid-shader</DcButton>)}</div><div className="relative w-full sm:w-72"><Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant" /><input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search real members..." className="w-full bg-surface border-2 border-outline-variant pl-9 pr-4 py-3 text-xs font-label-mono" /></div></div>
