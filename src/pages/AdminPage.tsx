...
export const AdminPage: React.FC = () => {
  ...
  const cards = [
    ['STUDENTS', overview?.students ?? 0, Users],
    ['MENTORS', overview?.mentors ?? 0, UserCheck],
    ['FACULTY', overview?.faculty ?? 0, ShieldCheck],
    ['ADMINS', overview?.admins ?? 0, ShieldCheck],
    ['POSTS', overview?.posts ?? 0, FileText],
    ['LIVE USERS', overview?.liveUsers ?? 0, Activity],
  ] as const;
  ...
  return <div className="space-y-8 pb-16">
    ...
    <section className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
      {cards.map(([label, value, Icon]) => <div key={label} className="border-2 border-outline-variant bg-surface p-5 shadow-[4px_4px_0_#171717]"><p className="font-label-mono text-[10px] text-on-surface-variant uppercase mb-2">{label}</p><div className="flex items-center justify-between"><strong className="dc-display text-2xl">{value}</strong><Icon className="w-4 h-4 text-primary" /></div></div>)}
    </section>
    <section className="border-2 border-outline-variant bg-surface shadow-[5px_5px_0_#171717]">
      <div className="p-5 border-b-2 border-outline-variant flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"><div><h3 className="dc-display text-3xl">MENTOR VERIFICATION.</h3><p className="font-label-mono text-[10px] uppercase text-on-surface-variant mt-1">Review official college applicants before granting mentor access</p></div><div className="flex gap-2">{(['pending','approved','rejected'] as const).map((status) => <button key={status} onClick={() => setStatusFilter(status)} className={`px-3 py-2 border-2 border-outline-variant font-label-mono text-[9px] uppercase ${statusFilter === status ? 'bg-primary text-on-primary' : 'bg-background'}`}>{status}</button>)}</div></div>
      <div className="p-5 space-y-4">
        {loading ? <div className="p-10 text-center"><Loader2 className="w-6 h-6 mx-auto animate-spin" /></div> : applications.length === 0 ? <div className="p-10 text-center border-2 border-dashed border-outline-variant"><CheckCircle2 className="w-7 h-7 mx-auto mb-3 text-on-surface-variant" /><p className="font-label-mono text-xs uppercase font-bold">NO {statusFilter.toUpperCase()} APPLICATIONS</p><p className="text-xs text-on-surface-variant mt-2">New authenticated college applications will appear here.</p></div> : applications.map((application) => <article key={application.id} className="border-2 border-outline-variant bg-background p-5"><div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-5"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="bg-dc-yellow border-2 border-outline-variant px-2 py-1 font-label-mono text-[9px] uppercase">{application.status}</span><span className="font-label-mono text-[9px] text-on-surface-variant">{new Date(application.created_at).toLocaleString()}</span></div><h4 className="dc-display text-3xl mt-3">{application.applicant_name}</h4><p className="text-sm mt-1">{application.applicant_email}</p><p className="text-xs text-on-surface-variant mt-1">{application.college} · {application.branch}</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => void openResume(application)} disabled={busyId === application.id} className="inline-flex items-center gap-2 px-3 py-2 border-2 border-outline-variant bg-surface font-label-mono text-[9px] uppercase"><ExternalLink className="w-3.5 h-3.5" /> Resume</button>{application.status === 'pending' && <><button type="button" onClick={() => void review(application, 'approve')} disabled={busyId === application.id} className="inline-flex items-center gap-2 px-3 py-2 border-2 border-outline-variant bg-dc-mint font-label-mono text-[9px] uppercase font-bold"><CheckCircle2 className="w-3.5 h-3.5" /> Approve</button><button type="button" onClick={() => void review(application, 'reject')} disabled={busyId === application.id} className="inline-flex items-center gap-2 px-3 py-2 border-2 border-outline-variant bg-dc-pink font-label-mono text-[9px] uppercase font-bold"><XCircle className="w-3.5 h-3.5" /> Reject</button></>}</div></div><div className="grid lg:grid-cols-3 gap-4 mt-5"><div><p className="font-label-mono text-[9px] uppercase text-on-surface-variant mb-1">Skills</p><p className="text-sm whitespace-pre-wrap">{application.skills}</p></div><div><p className="font-label-mono text-[9px] uppercase text-on-surface-variant mb-1">Experience</p><p className="text-sm whitespace-pre-wrap">{application.experience}</p></div><div><p className="font-label-mono text-[9px] uppercase text-on-surface-variant mb-1">Motivation</p><p className="text-sm whitespace-pre-wrap">{application.motivation}</p></div></div></article>)}
      </div>
    </section>
