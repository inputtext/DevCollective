...
import { Heart, MessageSquare, Plus, Send, Share2, X, Loader2, AlertTriangle } from 'lucide-react';

export const CommunityPage: React.FC = () => {
  const { user, posts, commentsByPost, addPost, toggleLikePost, loadPostComments, addPostComment } = useAuth();
  ...
  const handleSubmitComment = async (event: React.FormEvent) => {
    ...
    setCommentError(null);
    try {
      await addPostComment(openCommentsFor, commentText);
      setCommentText('');
    } catch (err: any) {
  ...
  const handleCommentLike = async (commentId: string) => {
    ...
    try {
      ...
      if (openCommentsFor) {
        // Update only the opened comment list; the server remains the source of truth.
  ...
  return <div className="space-y-8 pb-16">
    ...
    {activeCommentPost && <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-md"><div className="bg-surface border-2 border-outline-variant p-6 max-w-2xl w-full max-h-[82vh] flex flex-col shadow-[7px_7px_0_#171717] relative"><button onClick={() => setOpenCommentsFor(null)} className="absolute top-3 right-3 p-1" aria-label="Close comments"><X className="w-5 h-5" /></button><p className="font-label-mono text-[10px] uppercase text-on-surface-variant">COMMUNITY / COMMENTS</p><h3 className="dc-display text-3xl mt-2 pr-10">{activeCommentPost.title || 'DISCUSSION.'}</h3><p className="text-sm text-on-surface-variant mt-2 line-clamp-2">{activeCommentPost.content}</p><div className="mt-5 pt-4 border-t-2 border-outline-variant overflow-y-auto flex-1 space-y-4 min-h-0">{commentsLoading ? <div className="py-10 flex items-center justify-center gap-2 text-on-surface-variant"><Loader2 className="w-4 h-4 animate-spin" /> Loading comments...</div> : commentError && activeComments.length === 0 ? <div className="py-10 text-center text-error"><AlertTriangle className="w-6 h-6 mx-auto mb-2" /><p className="text-sm">{commentError}</p></div> : activeComments.length === 0 ? <div className="border-2 border-dashed border-outline-variant p-8 text-center"><MessageSquare className="w-6 h-6 mx-auto mb-2 text-primary" /><p className="font-label-mono text-[10px] uppercase font-bold">NO COMMENTS YET</p><p className="text-xs text-on-surface-variant mt-2">Start the discussion for this post.</p></div> : activeComments.map((comment) => <div key={comment.id} className="border-2 border-outline-variant/60 p-4 bg-surface-container-low"><div className="flex items-center gap-3">{comment.authorAvatar ? <img src={comment.authorAvatar} alt={comment.authorName} className="w-9 h-9 border border-outline-variant object-cover" /> : <div className="w-9 h-9 border border-outline-variant bg-dc-yellow flex items-center justify-center text-xs font-bold">{comment.authorName.slice(0,1)}</div>}<div><p className="text-sm font-bold">{comment.authorName}</p><p className="font-label-mono text-[9px] uppercase text-on-surface-variant">{comment.authorRole} · {comment.authorRep} REP · {new Date(comment.createdAt).toLocaleString()}</p></div></div><p className="mt-3 text-sm leading-relaxed whitespace-pre-wrap">{comment.content}</p><div className="mt-3 flex items-center"><button onClick={() => void handleCommentLike(comment.id)} disabled={commentLikeBusy === comment.id} className={`inline-flex items-center gap-1.5 font-label-mono text-[10px] uppercase ${comment.likedByMe ? 'text-primary' : 'text-on-surface-variant hover:text-primary'} disabled:opacity-50`}><Heart className="w-3.5 h-3.5" fill={comment.likedByMe ? 'currentColor' : 'none'} /> {comment.likes || 0}</button></div></div>)}</div>{commentError && activeComments.length > 0 && <p className="mt-3 text-xs text-error flex items-center gap-2"><AlertTriangle className="w-4 h-4" />{commentError}</p>}<form onSubmit={handleSubmitComment} className="mt-4 pt-4 border-t-2 border-outline-variant flex gap-3"><input value={commentText} onChange={(e) => setCommentText(e.target.value.slice(0, 2000))} placeholder="Write a comment..." className="flex-1 border-2 border-outline-variant bg-surface p-3 text-sm" disabled={commentSubmitting} /><button type="submit" disabled={!commentText.trim() || commentSubmitting} className="px-5 border-2 border-outline-variant bg-primary text-on-primary font-label-mono text-[10px] uppercase font-bold shadow-[3px_3px_0_#171717] disabled:opacity-50 flex items-center gap-2">{commentSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />} Comment</button></form></div></div>}
  </div>;
};
