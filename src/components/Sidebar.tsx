...
export const Sidebar: React.FC = () => {
  ...
  const levelItems: { id: PageTab; label: string; icon: React.ReactNode }[] = [{ id: 'level-0' as PageTab, label: 'Level 0', icon: <BookOpen className="h-[18px] w-[18px]" /> }];
  ...
  return <aside data-lenis-prevent-wheel className={['sticky top-0 z-30 hidden h-screen shrink-0 flex-col overflow-visible md:flex','bg-surface/95 backdrop-blur-sm border-r border-outline-variant/70','transition-[width] duration-300 ease-out',sidebarCollapsed ? 'w-[76px]' : 'w-[264px]'].join(' ')}>
    <div className="shrink-0 px-3 pb-4 pt-4">
      <div className={['flex items-center', sidebarCollapsed ? 'justify-center' : 'gap-2'].join(' ')}>
        <button onClick={() => setActiveTab(user ? 'dashboard' : 'landing')} title="DevCollective — Build. Scale. Lead." className={['group flex min-w-0 items-center rounded-xl text-left transition-colors',sidebarCollapsed ? 'justify-center' : 'flex-1 gap-3 px-2 py-2','hover:bg-surface-container-high/70'].join(' ')}><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-primary-container text-white shadow-[0_4px_12px_rgba(0,0,0,0.16)] transition-transform duration-200 group-hover:-translate-y-0.5"><Terminal className="h-[22px] w-[22px]" /></span>{!sidebarCollapsed && <span className="min-w-0 overflow-hidden"><span className="block truncate font-headline-md text-[17px] font-black leading-none tracking-[-0.04em] text-primary">DevCollective</span><span className="mt-1 block truncate font-label-mono text-[8px] uppercase tracking-[0.18em] text-outline">Build. Scale. Lead.</span></span>}</button>
    ...
    <nav className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-3 pb-4" aria-label="Main navigation"><div className="space-y-1">{renderGroup('Workspace', primary)}{renderGroup('Progress', levelItems)}{renderGroup('Connect', secondary)}</div></nav>
