...
export const MentorAccessModal: React.FC<MentorAccessModalProps> = ({ open, onClose }) => {
  ...
  const openApplication = () => {
    setNotice(null);
    setView('application');
    window.setTimeout(() => scrollRef.current?.scrollTo({ top: 0, behavior: 'instant' as ScrollBehavior }), 0);
  };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (Object.values(values).some((value) => !value)) {
      setNotice('Please complete every application field before submitting.');
      return;
    }
    if (!resume) {
      setNotice('Please attach your resume PDF before submitting.');
      return;
    }
    if (!SUPABASE_URL || !SUPABASE_PUBLISHABLE_KEY) {
      setNotice('Mentor applications are temporarily unavailable because the application service is not configured.');
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${SUPABASE_URL.replace(/\/$/, '')}/functions/v1/mentor-application`, {
        method: 'POST',
        headers: {
      ...
      const message = error instanceof Error ? error.message : 'We could not submit your application right now.';
      setNotice(message);
    } finally {
  ...
  return (
    <div
      className="fixed inset-0 z-[120] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 sm:p-6 overflow-hidden"
      role="dialog"
      aria-modal="true"
      aria-labelledby="mentor-access-title"
      onMouseDown={(event) => { if (event.target === event.currentTarget && !isSubmitting) onClose(); }}
    >
      <div
        ref={scrollRef}
        onWheel={handleModalWheel}
        className="w-full max-w-3xl max-h-[90vh] overflow-y-auto overscroll-contain touch-pan-y border-2 border-outline-variant bg-surface dc-hard-shadow"
      ...
      >
        <div className="sticky top-0 z-10 flex items-center justify-between border-b-2 border-outline-variant bg-surface px-5 py-4 sm:px-7">
          ...
          <button type="button" onClick={onClose} disabled={isSubmitting} aria-label="Close mentor access dialog" className="p-2 border-2 border-outline-variant hover:bg-dc-pink transition-colors disabled:opacity-50"><X className="w-5 h-5" /></button>
        </div>
            ...
            <div>
              ...
              <div className="grid sm:grid-cols-2 gap-4 mt-3">
                <button type="button" onClick={openApplication} className="text-left border-2 border-outline-variant bg-background hover:bg-dc-mint p-5 transition-colors dc-hard-shadow-sm">
                  ...
                  <p className="mt-2 text-sm text-on-surface-variant">Submit your profile, experience, skills, and resume directly to the DevCollective developer team.</p>
                </button>
        ...
        {view === 'application' && (
          <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-6">
            ...
            <div className="p-4 sm:p-5 bg-dc-mint border-2 border-outline-variant text-[#171717]">
              <div className="flex items-start gap-3"><FileText className="w-5 h-5 shrink-0 mt-0.5" /><p className="text-sm leading-relaxed">Complete every field and attach your PDF resume. When you submit, DevCollective sends the application directly to the developer team; no mail client is required.</p></div>
            </div>
            <div className="grid sm:grid-cols-2 gap-5">
              {[
                ...
                <div key={field} className="space-y-2">
                  <label className="dc-mono text-[9px] uppercase tracking-[0.16em] text-on-surface-variant">{label} <span className="text-primary">*</span></label>
                  <input type={field === 'email' ? 'email' : 'text'} value={draft[field as keyof ApplicationDraft]} onChange={(event) => updateDraft(field as keyof ApplicationDraft, event.target.value)} placeholder={placeholder} required aria-required="true" className="w-full bg-background border-2 border-outline-variant px-4 py-3 text-sm text-on-surface placeholder:text-on-surface-variant/50 focus:outline-none focus:border-primary" />
                </div>
              ))}
              <div className="space-y-2 sm:col-span-2">
                <label className="dc-mono text-[9px] uppercase tracking-[0.16em] text-on-surface-variant">Skills <span className="text-primary">*</span></label>
                <input value={draft.skills} onChange={(event) => updateDraft('skills', event.target.value)} placeholder="React, Node.js, Python, system design..." required aria-required="true" className="w-full bg-background border-2 border-outline-variant px-4 py-3 text-sm text-on-surface placeholder:text-on-surface-variant/50 focus:outline-none focus:border-primary" />
              </div>
              <div className="space-y-2 sm:col-span-2">
                <label className="dc-mono text-[9px] uppercase tracking-[0.16em] text-on-surface-variant">Mentoring / Technical Experience <span className="text-primary">*</span></label>
                <textarea value={draft.experience} onChange={(event) => updateDraft('experience', event.target.value)} placeholder="Projects, internships, open source, teaching, leadership..." rows={4} required aria-required="true" className="w-full bg-background border-2 border-outline-variant px-4 py-3 text-sm text-on-surface placeholder:text-on-surface-variant/50 focus:outline-none focus:border-primary resize-y" />
              </div>
              <div className="space-y-2 sm:col-span-2">
                <label className="dc-mono text-[9px] uppercase tracking-[0.16em] text-on-surface-variant">Why do you want to mentor? <span className="text-primary">*</span></label>
                <textarea value={draft.message} onChange={(event) => updateDraft('message', event.target.value)} placeholder="Tell the developer team what you can contribute to the collective." rows={4} required aria-required="true" className="w-full bg-background border-2 border-outline-variant px-4 py-3 text-sm text-on-surface placeholder:text-on-surface-variant/50 focus:outline-none focus:border-primary resize-y" />
              </div>
            </div>
            <div className="border-2 border-dashed border-outline-variant bg-background p-5">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-sm">Resume PDF <span className="text-primary">*</span></p>
                  <p className="text-xs text-on-surface-variant mt-1">Required · PDF only · up to 5 MB.</p>
                  <label className="inline-flex items-center gap-2 mt-4 border-2 border-outline-variant bg-surface px-4 py-2.5 text-xs font-bold uppercase cursor-pointer hover:bg-dc-blue transition-colors"><Upload className="w-4 h-4" /> Choose PDF<input type="file" accept="application/pdf,.pdf" onChange={handleResume} required className="hidden" /></label>
                  {resume && <p className="mt-3 text-xs dc-mono flex items-center gap-2"><FileText className="w-4 h-4" /> {resume.name}</p>}
                </div>
              </div>
            </div>
            ...
            <div className="border-t-2 border-outline-variant pt-6 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
              <p className="text-xs text-on-surface-variant">Your resume is securely sent with the application. Nothing is opened in your email app.</p>
              <button type="submit" disabled={isSubmitting} className="shrink-0 inline-flex items-center justify-center gap-3 bg-primary text-on-primary border-2 border-outline-variant px-6 py-4 font-bold uppercase dc-hard-shadow-sm disabled:opacity-50"><Send className="w-4 h-4" /> {isSubmitting ? 'SUBMITTING...' : 'SUBMIT APPLICATION'}</button>
            </div>
          </form>
        )}
        ...
        {view === 'sent' && (
          <div className="p-6 sm:p-8 space-y-7">
            <div className="p-5 sm:p-6 bg-dc-mint border-2 border-outline-variant text-[#171717]">
              <div className="flex items-start gap-4"><CheckCircle2 className="w-7 h-7 shrink-0 mt-0.5" /><div><p className="dc-mono text-[10px] uppercase tracking-[0.16em] font-bold">APPLICATION SENT</p><h3 className="dc-display text-3xl mt-2">WE RECEIVED IT.</h3><p className="mt-3 text-sm leading-relaxed">Your mentor application and resume were sent successfully to the DevCollective developer team. They can review your information and contact you for the next step.</p></div></div>
            </div>
            <button type="button" onClick={onClose} className="w-full border-2 border-outline-variant bg-background hover:bg-dc-blue px-5 py-3 font-bold uppercase dc-mono text-xs">Close</button>
          </div>
        )}
      </div>
    </div>
