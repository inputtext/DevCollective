...
export function registerLearningProgressRoutes(
  ...
  app.get('/api/learning/level-0', requireAuth, async (req, res) => {
    try {
      ...
      return res.json({
        ...
        progress,
        moduleProgress,
        levelProgress: Array.isArray(levelProgress) ? levelProgress[0] ?? null : levelProgress,
      });
    } catch (error: any) {
  ...
  app.post('/api/learning/level-0/submodules/:submoduleId/verify', requireAuth, async (req, res) => {
    try {
      ...
      if (message.includes('Unknown learning submodule')) {
        return res.status(404).json({ error: 'Learning submodule not found.' });
      }
