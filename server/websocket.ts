...
async function authenticate(token: string): Promise<string | null> {
  try {
    ...
    const result = await verifyToken(token, { secretKey: process.env.CLERK_SECRET_KEY, ...(authorizedParties?.length ? { authorizedParties } : {}) });
...
async function ensureDirectConversation(userA: string, userB: string) {
  ...
  if (!userB || userA === userB) throw new Error('You cannot message yourself.');
  const { data: recipient, error: recipientError } = await supabaseAdmin.from('devcollective_profiles').select('clerk_user_id').eq('clerk_user_id', userB).maybeSingle();
  if (recipientError) throw recipientError;
  if (!recipient) throw new Error('That DevCollective member could not be found.');
  const { data: memberships, error: membershipError } = await supabaseAdmin.from('devcollective_conversation_members').select('conversation_id,clerk_user_id').in('clerk_user_id', [userA, userB]);
  if (membershipError) throw membershipError;
  const counts = new Map<string, Set<string>>();
  for (const row of memberships || []) { const set = counts.get(row.conversation_id) || new Set<string>(); set.add(row.clerk_user_id); counts.set(row.conversation_id, set); }
  for (const [conversationId, members] of counts) if (members.size === 2 && members.has(userA) && members.has(userB)) return conversationId;
  const { data: conversation, error: conversationError } = await supabaseAdmin.from('devcollective_conversations').insert({ kind: 'direct', created_by: userA }).select('id').single();
  if (conversationError || !conversation) throw conversationError || new Error('Could not create conversation.');
  const { error: memberError } = await supabaseAdmin.from('devcollective_conversation_members').insert([{ conversation_id: conversation.id, clerk_user_id: userA }, { conversation_id: conversation.id, clerk_user_id: userB }]);
  if (memberError) throw memberError;
  return conversation.id as string;
}

async function hiddenMessageIds(userId: string, conversationId?: string) {
  if (!supabaseAdmin) throw new Error('Supabase is not configured.');
  let query = supabaseAdmin.from('devcollective_message_deletions').select('message_id').eq('clerk_user_id', userId);
  if (conversationId) query = query.in('message_id', (await supabaseAdmin.from('devcollective_messages').select('id').eq('conversation_id', conversationId)).data?.map((row) => row.id) || ['__none__']);
...
async function latestVisibleMessage(conversationId: string, userId: string) {
  const { data, error } = await supabaseAdmin!.from('devcollective_messages').select('id,body,created_at,deleted_at').eq('conversation_id', conversationId).order('created_at', { ascending: false }).limit(50);
  if (error) throw error;
  const hidden = await hiddenMessageIds(userId, conversationId);
  const message = (data || []).find((item: any) => !hidden.has(item.id));
  if (!message) return null;
  return { body: message.deleted_at ? 'Message deleted' : message.body, createdAt: message.created_at, deleted: Boolean(message.deleted_at) };
}

async function loadConversationList(userId: string) {
  if (!supabaseAdmin) throw new Error('Supabase is not configured.');
  const { data: memberships, error } = await supabaseAdmin.from('devcollective_conversation_members').select('conversation_id,last_read_at,devcollective_conversations(id,kind,created_at,updated_at,last_message_at)').eq('clerk_user_id', userId).order('last_message_at', { ascending: false, foreignTable: 'devcollective_conversations' });
  if (error) throw error;
  const conversationIds = (memberships || []).map((row: any) => row.conversation_id);
  if (!conversationIds.length) return [];
  const { data: members, error: memberError } = await supabaseAdmin.from('devcollective_conversation_members').select('conversation_id,clerk_user_id').in('conversation_id', conversationIds);
  if (memberError) throw memberError;
...
async function handleEvent(client: Client, event: IncomingEvent) {
  switch (event.type) {
    ...
    case 'conversation:open': {
      ...
      const messages = await loadMessages(event.conversationId, client.userId);
      await markConversationRead(event.conversationId, client.userId);
      send(client.socket, 'conversation:history', { conversationId: event.conversationId, messages });
      return;
    }
    ...
    case 'message:send': {
      ...
      if (event.replyToMessageId) {
        const { data: reply, error: replyError } = await supabaseAdmin!.from('devcollective_messages').select('id').eq('id', event.replyToMessageId).eq('conversation_id', event.conversationId).maybeSingle();
    ...
    case 'message:edit': {
      const body = String(event.body || '').trim();
      if (!event.messageId || !body) throw new Error('messageId and message body are required.');
      if (body.length > 4000) throw new Error('Message is too long.');
      const { data: message, error: lookupError } = await supabaseAdmin!.from('devcollective_messages').select('id,conversation_id,sender_clerk_user_id,deleted_at').eq('id', event.messageId).maybeSingle();
    ...
    case 'message:delete': {
      ...
      const { data: message, error: lookupError } = await supabaseAdmin!.from('devcollective_messages').select('id,conversation_id,sender_clerk_user_id,deleted_at').eq('id', event.messageId).eq('conversation_id', event.conversationId).maybeSingle();
      if (lookupError) throw lookupError;
      if (!message) throw new Error('Message not found.');
      if (message.deleted_at) throw new Error('Message is already deleted.');
      const mode = event.body === 'for_me' ? 'for_me' : event.body === 'for_everyone' ? 'for_everyone' : '';
      if (mode === 'for_everyone') {
        if (message.sender_clerk_user_id !== client.userId) throw new Error('Only the sender can delete a message for everyone.');
        ...
        if (error) throw error;
        broadcast('message:deleted', { conversationId: message.conversation_id, messageId: message.id, scope: 'for_everyone', deletedAt }, (other) => other.userId === client.userId || other.conversationId === message.conversation_id);
        return;
      }
      if (mode === 'for_me') {
        ...
        if (error) throw error;
        send(client.socket, 'message:deleted', { conversationId: message.conversation_id, messageId: message.id, scope: 'for_me' });
        return;
      }
      throw new Error('Choose a valid delete option.');
    }
    case 'message:read': {
      ...
      const now = new Date().toISOString();
      await supabaseAdmin!.from('devcollective_messages').update({ read_at: now }).eq('id', event.messageId).eq('conversation_id', event.conversationId).neq('sender_clerk_user_id', client.userId);
      await supabaseAdmin!.from('devcollective_conversation_members').update({ last_read_at: now }).eq('conversation_id', event.conversationId).eq('clerk_user_id', client.userId);
      broadcast('message:read', { conversationId: event.conversationId, messageId: event.messageId, userId: client.userId, readAt: now }, (other) => other.conversationId === event.conversationId);
      return;
    }
    case 'typing:start':
    case 'typing:stop': {
...
export function startWebSocketServer() {
  ...
  wss.on('connection', (socket) => {
    let client: Client | null = null; let authenticated = false;
    const authTimeout = setTimeout(() => { if (!authenticated) socket.close(1008, 'Authentication timeout'); }, 8000);
